
import './App.css';
import { HamburgerIcon, TriangleDownIcon } from '@chakra-ui/icons'
import {
  Menu,
  MenuButton,
  MenuList,
  MenuItem,Input,
  IconButton,Divider,Center,Icon,Avatar,Grid,GridItem,Stack
  
} from '@chakra-ui/react'
import { Flex, Spacer ,Box,Heading,Button,ButtonGroup,Image} from '@chakra-ui/react'
import {  HiOutlineQuestionMarkCircle,HiOutlineUsers} from 'react-icons/hi'
import {  TiBell} from 'react-icons/ti'
import {  BiTimeFive,BiUserCircle} from 'react-icons/bi'
import {  GoCalendar,GoGraph} from 'react-icons/go'
import {  GrDocumentText} from 'react-icons/gr'
import {  BsTag} from 'react-icons/bs'
import {  FaRegCalendarAlt} from 'react-icons/fa'
import {  FiSettings} from 'react-icons/fi'




function App() {
  return (
    
    <div className="App">
     
      <Flex bg="" border="1px solid #D3D3D3" minWidth='max-content' height="50pxpx" alignItems='center' gap='2'>
      <Box>
  <Button bg="white" 
  _hover={{bg:'white'}}
    as={IconButton}
    aria-label='Options'
    icon={<HamburgerIcon />}
  />
</Box>
  <Box p='2' >
    <Image width="120px" src="data:image/png;base64,"></Image>
  </Box>
  <Spacer />
  <Menu  >
  <MenuButton bg='white'  _hover={{bg:'white'}} as={Button} rightIcon={<TriangleDownIcon />} fontSize="13px">
    Harshini Usarthi's...
  </MenuButton>
  <MenuList>
    <MenuItem>Download</MenuItem>
    <MenuItem>Create a Copy</MenuItem>
  
  </MenuList>
</Menu>
  <ButtonGroup gap='2' >
    <Button colorScheme='blue'  variant='outline' fontSize="13px" size="xs" mt="12px" borderRadius="5px">UPGRADE</Button>
    <Center height='50px'>
  <Divider orientation='vertical' />
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={HiOutlineQuestionMarkCircle}/></Button>
  <Divider orientation='vertical' />
  <Button bg="white" _hover="bg:white" width="60px"> <Icon as={TiBell}/></Button>
  <Divider orientation='vertical' />
  <Button bg="white" _hover="bg:white" width="60px"> <Avatar name='Harshini Usarthi' size="sm" src=''/> </Button>
  </Center>
    
  </ButtonGroup>
</Flex>
<Grid
  height="550px"
  templateRows='repeat(15, 1fr)'
  templateColumns='repeat(16, 1fr)'
  gap={0}
>
  <GridItem rowSpan={15} colSpan={1} bg='' border="1px solid #D3D3D3">

  <Stack direction='column' spacing={4} align='center'>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={BiTimeFive}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={GoCalendar}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={FaRegCalendarAlt}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={GoGraph}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={GrDocumentText}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={HiOutlineUsers}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={BiUserCircle}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={BsTag}/></Button>
  <Button bg="white" _hover="bg:white" width="60px"><Icon  as={FiSettings}/></Button>
  
 
</Stack>
</GridItem>
<GridItem rowSpan={1} colSpan={15} bg='' mx="20px">
 

</GridItem>
<GridItem rowSpan={2} colSpan={15} bg='' mx="20px"   border="1px solid #D3D3D3" >
 
<Input  placeholder='What are you working on?' size='md' width="80%" float="left" m="15px" borderRadius="2px"/>
<Button m="15px" bg="white" _hover="bg:white" color="blue">Project</Button>
</GridItem>
<GridItem rowSpan={2} colSpan={15} bg='' mx="20px"  >
 

</GridItem>

<GridItem rowSpan={10} colSpan={15} bg='' mx="20px"  border="1px solid #D3D3D3" ml="400px" mt="50px">
 <Box border="1px solid #D3D3D3" width="30% " display="flex" flexDirection="column" justifyItems="baseline">
 <Image  width="120px" src="https://app.clockify.me/assets/ui-icons/empty-tracker-icon.webp"/>
 <Heading as='h4' size='md' >
    Let's start tracking
  </Heading>
 </Box>

</GridItem>
  
 </Grid>
</div>

    
    
  );
}

export default App;
